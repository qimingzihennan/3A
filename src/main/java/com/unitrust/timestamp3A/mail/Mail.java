...
public class Mail {
	...
	public void setPros(Map<String, String> map) {
		...
		for (Map.Entry<String, String> entry : map.entrySet()) {
			pros.put(entry.getKey(), entry.getValue());
		}
	 ...
	 * @throws IOException
	 */
	public void setMultipartsWithFileName(List<String> fileList) throws MessagingException, IOException {
		if (multipart == null) {
			multipart = new MimeMultipart();
		}
	 ...
	 * @throws IOException
	 */
	public void setMultiparts(List<File> fileList) throws MessagingException, IOException {
		if (multipart == null) {
			multipart = new MimeMultipart();
		}
		for (int i = 0; i < fileList.size(); i++) {
			bodypart = new MimeBodyPart();
			DataSource dataSource = new FileDataSource(fileList.get(i));
			bodypart.setDataHandler(new DataHandler(dataSource));
			// 文件名要加入编码，不然出现乱码
			bodypart.setFileName(MimeUtility.encodeText(fileList.get(i).getName()));
